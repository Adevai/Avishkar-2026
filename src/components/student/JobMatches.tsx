            ...
            >
                    ...
                    <div className="mb-3 flex items-center gap-2">
                      ...
                      {(dl.urgency === 'critical' || dl.urgency === 'closed') && (
                        <span className="text-[10px] text-rose-600 font-bold uppercase tracking-wider">
                          Act now — apply before {job.deadline}
                        </span>
                      )}
                    </div>
                  );
              ...
              <div className="pt-3 border-t border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div className="text-[10px] text-slate-400">
                  <span>Min CGPA: <strong>{job.minCgpa}</strong></span> • <span>Deadline: {job.deadline}</span>
                </div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <label
                    className={`flex items-center gap-2 px-3 py-2 rounded-xl border text-xs font-bold cursor-pointer transition-all select-none shadow-xs ${
                      job.hasApplied
                        ? 'bg-emerald-50 text-emerald-800 border-emerald-300 ring-2 ring-emerald-500/20'
                        : 'bg-slate-50 text-slate-700 border-slate-200 hover:bg-blue-50/70 hover:border-blue-300'
                    }`}
                    title={job.hasApplied ? "Click to unmark application" : "Check to mark this job as applied"}
                  >
                    <input
                      type="checkbox"
